...
import { CtaContent, defaultHomeContent } from "@/app/lib/cms/homeContent";

type Props = { content?: CtaContent };

export default function CTASection({ content = defaultHomeContent.cta }: Props) {
  return (
    <section className="relative overflow-hidden bg-[#000] py-28">
      ...
      <div className="relative z-10 mx-auto max-w-3xl px-6 text-center lg:px-8">
        <motion.div
          initial={{ opacity: 0, y: 35, scale: 0.82, rotate: -8, filter: "blur(10px)" }}
        ...
        >
          <motion.img
            ...
            animate={{ y: [-8, 8, -8], rotate: [-3, 3, -3], scale: [1, 1.035, 1] }}
        ...
        >
          <a href={content.primaryHref} className="group inline-flex items-center gap-2.5 rounded-lg bg-hook-red px-10 py-4 text-lg font-bold text-white transition-all duration-300 hover:bg-hook-red-light hover:shadow-[0_0_35px_rgba(200,0,0,0.35)]">
            {content.primaryLabel}
            <ArrowLeft className="h-4 w-4 transition-transform duration-300 group-hover:-translate-x-1" />
          </a>

          <a href={content.secondaryHref} target={content.secondaryHref.startsWith("http") ? "_blank" : undefined} rel="noopener noreferrer" className="inline-flex items-center gap-2.5 rounded-lg border border-white/10 px-10 py-4 text-lg font-bold text-white transition-all duration-300 hover:border-white/20 hover:bg-white/[0.03]">
